...
fn main() -> io::Result<()> {
  ...
  for line in reader.lines() {
    ...
    if i < 9 {
      ...
      let amount = caps.get(1).unwrap().as_str().parse::<i32>().unwrap() as usize;
      ...
      let new_from_length = from_crate.len().saturating_sub(amount);
      let moving_crates: Vec<char> = from_crate.drain(new_from_length..).collect();
      to_crate = [to_crate, moving_crates].concat();
