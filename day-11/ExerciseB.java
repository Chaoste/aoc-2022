...
class Monkey {
  ...
  void inspect(List<Monkey> monkeys) {
    while (this.items.size() > 0) {
      ...
      BigInteger newPrio = this.prioOp.apply(item).mod(this.superModulo);
...
public class ExerciseB {
  ...
  public static void main(String[] args) {
    ...
    BigInteger superModulo = BigInteger.valueOf(1);
    for (int i = 0; i < lines.size() - 1; i += 7) {
      ...
      superModulo = superModulo.multiply(divisibleBy);
      ...
      if (line.startsWith("Monkey")) {
        Monkey newMonkey = new Monkey(initialItems, prioOp, passOp, superModulo);
    ...
    for (int j = 0; j < n_monkeys; j++) {
      monkeys.get(j).superModulo = superModulo;
    }
    ...
    BigInteger monkeyBusiness = BigInteger.valueOf(largest[0]).multiply(BigInteger.valueOf(largest[1]));
