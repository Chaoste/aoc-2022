...
while (($line = fgets($handle)) !== false) {
  ...
  $map[] = $result;
  if (($start_index = strpos($line, 'S')) !== false) {
    $start[0] = count($map) - 1;
    $start[1] = $start_index;
  }
  if (($end_index = strpos($line, 'E')) !== false) {
    $end[0] = sizeof($map) - 1;
    $end[1] = $end_index;
  }
}
...
while ($path === null) {
  ...
  foreach ($neighbours as [$row, $col]) {
    ...
    if ($map[$pos[0]][$pos[1]] == ord("S") && $map[$row][$col] > ord("b")) continue; // S=a
    if ($map[$row][$col] == ord("E") && $map[$pos[0]][$pos[1]] < ord("y")) continue; // E=z
